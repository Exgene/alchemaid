...
<div class="row-span-1 flex h-[90vh] flex-col items-center justify-center p-4 ">
	<!-- Reports Part -->
	<div class="flex lg:w-1/2 flex-col rounded-sm p-1">
		<Dialog.Root>
			...
			<div class="mt-4 gap-x-3 overflow-auto rounded-md border-2 border-gray-200 bg-gray-100 p-2 shadow-inner dark:border-black dark:bg-gray-800">
				...
				{#each $userPrivateData?.reports ?? [] as report, i}
					<a href={`dashboard/${report.title}`}>
						<Card.Root class="mb-2 flex justify-between items-center">
							<Card.Header>
							...
							<Card.Content class="pt-6">
								<Button class='bg-red-500' on:click={()=>{}}> X </Button>
							</Card.Content>
						</Card.Root>
					</a>
				{/each}
	...
	<!-- Your Doctor Part -->
	<div class="mb-12 mt-4 w-1/2 self-center rounded-sm p-1">
